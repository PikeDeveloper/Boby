        ...
        return Row(
            ...
            children: [
              _opItem(
                ...
                onChanged: (v) => StorageService.instance.setMathOpAdd(v ?? false),
              ),
              _opItem(
                context,
                label: '-',
                value: StorageService.instance.getMathOpSub(),
                onChanged: (v) => StorageService.instance.setMathOpSub(v ?? false),
              ),
              _opItem(
                context,
                label: 'X',
                value: StorageService.instance.getMathOpMul(),
                onChanged: (v) => StorageService.instance.setMathOpMul(v ?? false),
              ),
              _opItem(
                context,
                label: '/',
                value: StorageService.instance.getMathOpDiv(),
                onChanged: (v) => StorageService.instance.setMathOpDiv(v ?? false),
              ),
            ],
          );
      },
  ...
  Widget _opItem(BuildContext context, {required String label, required bool value, required ValueChanged<bool?> onChanged}) {
    return Container(
      ...
      child: CheckboxListTile(
        title: Text(label,style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.indigo),),
        value: value,
        onChanged: onChanged,
